...
func GetJs() func(c *fiber.Ctx) (err error) {
	return func(c *fiber.Ctx) (err error) {
		...
		if file := fmt.Sprintf("%s", c.Params("file")); strings.HasSuffix(file, ".js") {
			...
			err = c.SendFile(fmt.Sprintf("%s/%s/static/%s/%s.js", pathBase, path[1], path[2], c.Params("file")))
		}
		if err != nil {
			return err
		}
...
func GetAsset() func(c *fiber.Ctx) (err error) {
	return func(c *fiber.Ctx) (err error) {
		path := strings.Split(c.Path(), "/")
		if path[1] == "assets" {
...
func GetHTML(fn templ.Component) func(c *fiber.Ctx) (err error) {
	return func(c *fiber.Ctx) (err error) {
		c.Set("Content-Type", fiber.MIMETextHTML)

		var output bytes.Buffer

		err = fn.Render(context.Background(), &output)
		if err != nil {
			return err
		}
